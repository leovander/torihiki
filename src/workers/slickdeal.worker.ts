...
const MESSAGE_THREAD_IDS = process.env.TELEGRAM_SLICKDEAL_MESSAGE_THREAD_IDS ? parseTelegramThreadIds(process.env.TELEGRAM_SLICKDEAL_MESSAGE_THREAD_IDS) : [];
...
export const worker: LocalWorker<any> = new LocalWorker<SLICKDEAL_MESSAGE|SLICKDEAL_REPEATABLE>(
    QUEUE_NAME,
    MESSAGE_THREAD_IDS,
    {
    ...
    async (job: Job<SLICKDEAL_MESSAGE|SLICKDEAL_REPEATABLE>) => {
        ...
        if ('link' in slickdealData) {
            telegramResponse = await telegramBot.telegram.sendMessage(
                ...
                {
                    reply_parameters: {
                        message_id: worker.telegramThreadIds[slickdealData.category]
                    }
                }
            ).catch(async (err) => {
                ...
                if (error.code === 429) {
                    ...
                    if (match) {
                        ...
                        logger.error(`Rate Limited for ${duration} while processing message - ${job.queueName}:${job.name}:${job.id}`);
