import { Job, JobsOptions, MetricsTime, Queue, QueueOptions, Worker } from "bullmq";
import { logger } from "./logger";
import { CommandInteractionOptionResolver } from "discord.js-selfbot-v13";

const DEFAULT_QUEUE_REMOVE_ON_COMPLETE_AGE = process.env.REMOVE_ON_COMPLETE_AGE ? parseInt(process.env.REMOVE_ON_COMPLETE_AGE) : 3600;
const DEFAULT_QUEUE_REMOVE_ON_COMPLETE_COUNT = process.env.REMOVE_ON_COMPLETE_COUNT ? parseInt(process.env.REMOVE_ON_COMPLETE_COUNT) : 1000;
const DEFAULT_QUEUE_REMOVE_ON_FAIL_COUNT = process.env.REMOVE_ON_FAIL_COUNT ? parseInt(process.env.REMOVE_ON_FAIL_COUNT) : 5000;
const DEFAULT_ATTEMPTS = process.env.ATTEMPTS ? parseInt(process.env.ATTEMPTS) : 3;
const DEFAULT_BACKOFF_DELAY = process.env.BACKOFF_DELAY ? parseInt(process.env.BACKOFF_DELAY) : 1000;
...
export class LocalWorker<DataType> {
  ...
  constructor(queueName: string, telegramThreadIds: TelegramThread[], queueOptions: QueueOptions, process: (job: Job<DataType>) => Promise<number | undefined>, jobOptions?: JobsOptions) {
    ...
    this.worker.on('failed', (job: Job | undefined, error: Error) => {
        if (job) {
            logger.error(`Message Failed - ${job.queueName}:${job.id} - ${error}`);
            //TODO: Send message to Admin, via telegram or pushover:
        }
    });

    this.worker.on('completed', (job: Job, returnvalue: number) => {
        logger.info(`Message Completed - ${job.queueName}:${job.id}:telegram:${returnvalue}`);
